...
def filter_reccos(recco_items, recco_ids, filter_criteria, df):
    ...
    INPUTS:
        ...
        filter_criteria: Dictionary where key is the column name in the df to filter on, 
            and values is a list of accepted values in that col
...
def make_X_df(df, W_df, features_list, feature_weights, topics_weight):
    ''' Creates an X dataframe with the set of features including topics from the W matrix df, 
...
def scale_cols(df, weights):
    ''' Scaler divides each value by the max of that column and multiplies by the feature weight
     (rather than using a standard scaler). All features range from 0 to a max value, 
...
if __name__ == '__main__':
    ...
    # Try it out with a random input trail
    ...
    # Try out the user profile recommender with a random set of trails
    rand_n = randint(2, 10)
